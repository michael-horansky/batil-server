...
class TreeDocumentViewer(HTMLObject):
    ...
    def __init__(self, iden, bp, home_index, client_privilege):
        super().__init__()
        self.iden = iden
        self.bp = bp
    ...
    def load_content(self):
        ...
        if self.current_display == "_ROOT_":
            self.display_element = db.execute("SELECT CHAPTER_ID, LABEL, CONTENT, NEXT_CHAPTER, FIRST_SUBCHAPTER, PARENT_CHAPTER FROM BOC_TREE_DOCUMENTS WHERE VIEWER = ? AND PARENT_CHAPTER = NULL", (self.iden,)).fetchone()
    ...
    def chapter_href(self, chapter_id):
        return(url_for(f"{self.bp}.{self.home_index}", **{f"{self.iden}_chapter" : chapter_id}))
        ...
        if self.display_parent is not None:
            ...
            while(cur_sibling is not None):
                if cur_sibling == self.current_display:
                    ...
                    a_add = ""
                self.structured_html.append(f"      <li><a href=\"{self.chapter_href(self.display_siblings[cur_sibling]["CHAPTER_ID"])}\"{a_add}>{self.display_siblings[cur_sibling]["LABEL"]}</a></li>")
                cur_sibling = self.display_siblings[cur_sibling]["NEXT_CHAPTER"]
        else:
            # Root has no siblings since tree is always connected
            self.structured_html.append(f"      <li><a href=\"{self.chapter_href(self.display_element["CHAPTER_ID"])}\">{self.display_element["LABEL"]}</a></li>")
    ...
    def make_main_content(self):
        if self.client_privilege == "ADMIN":
            self.structured_html.append([
                ...
                f"    <form id=\"{self.iden}_form\" class=\"tdv_form\" action=\"{url_for(f"{self.bp}.action_{self.iden}")}\">",
                f"      <div id=\"{self.iden}_tabs\" class=\"tdv_tabs\">",
                f"        <div id=\"{self.iden}_tab_read\" class=\"tdv_tab active\">Read</div>",
                f"        <div id=\"{self.iden}_tab_edit\" class=\"tdv_tab\">Edit</div>",
                f"      </div>",
                ...
                f"        <button type=\"submit\" class=\"tdv_action_button\" name=\"{self.iden}_action\" value=\"insert_new_chapter_next\">Insert new chapter next</button>",
                f"        <button type=\"submit\" class=\"tdv_action_button\" name=\"{self.iden}_action\" value=\"insert_new_child\">Add new child</button>",
                f"      </div>",
                f"      <input type=\"hidden\" name=\"{self.iden}_open_chapter\" value=\"{self.display_element["CHAPTER_ID"]}\">",
                f"    </form>",
                f"  </div>"
                ])
    ...
    def make_right_sidebar(self):
        ...
        while(cur_child is not None):
            self.structured_html.append(f"      <li><a href=\"{self.chapter_href(self.display_children[cur_child]["CHAPTER_ID"])}\"{a_add}>{self.display_children[cur_child]["LABEL"]}</a></li>")
